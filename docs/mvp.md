...
Resource notes may store a `credential_ref` Keychain pointer (never the secret). Native login dialogs and Chromium HTTP basic/digest (browser chrome, not DOM) use `fill_credential`; the launcher collects username/password. HTTP basic/digest **file servers** (listings and downloads) use `fetch_url`: unauthenticated HEAD first, then `fetch_url` again with `credential_ref` so the host can send Digest/Basic — not the browser, and not `curl --user`. **Save in Keychain** is off by default and only overwrites a ref that already exists on a vault note. Values never go to the model, logs, receipts, or the WebView.
