...
- **Attachments** — a paperclip on the compact and conversation bars opens a macOS file sheet (images and videos, multiple). ⌘V in the textarea attaches a clipboard image (png/jpeg/gif/webp). Chips show the file name only; click to remove. An empty prompt is allowed when an attachment (or mention) is present. Images go to the model as vision; videos are copied into scratch `input/` and, on macOS, a best-effort poster frame is attached as vision. **New** clears pending chips; hiding the launcher keeps them. Drag-and-drop is later.
