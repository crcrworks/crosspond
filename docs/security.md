...
| `skill_install` | Fetch and scan first (no write). Scanner `fail` is a tool error in every mode, including Auto — no Allow card. `warn` / `unknown` always RequireApproval, including Auto. `pass` follows ExternalWrite, except tainted tasks still need Allow |
