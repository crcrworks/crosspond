...
When a Resource note has `credential_ref`, the agent uses that pointer — never a username or password. Native login dialogs use `fill_credential` with Accessibility node ids. HTTP basic/digest **file servers** use `fetch_url` (unauthenticated HEAD, then the same URL plus `credential_ref`); the host sends Digest or Basic from Keychain / the login card. Chromium HTTP basic/digest in an already-open tab (browser chrome, not the page DOM) uses `browser_navigate` then `fill_credential` with **only** `credential_ref`. On a Keychain miss the launcher shows Username / Password and an optional **Save in Keychain** switch (default off, offered only if that ref already exists on a vault note). Fill sends `submit_credential` into Rust. Values must not appear in `AgentEvent`, the WebView, logs, `events.jsonl`, `session.json`, receipts, or tool results. A Keychain hit in Manual/AI shows Allow (“fill saved login”) for `fill_credential`; `fetch_url` stays read-only and the login card is consent. Auto fills without asking. `run_command` refuses curl `--user` / `--digest` / `--basic` / `--ntlm` and `user:pass@` URLs (including `smb://`) without echoing the command. Do not interpolate secrets into `run_command` or `smb://user:pass@host`. Host HTTP auth must not put the password on a process command line.
