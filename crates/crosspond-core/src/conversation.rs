...
/// Drop secret-shaped keys the model must never pass on login tools.
pub(crate) fn redact_sensitive_tool_arguments(calls: &mut [ToolCall]) {
    for call in calls {
        if call.name != "fill_credential" && call.name != "fetch_url" {
            continue;
        }
...
mod tests {
    ...
    fn redact_fill_credential_drops_secret_keys() {
        ...
        assert!(calls[1].arguments.contains("lab.fileserver"));
        assert!(calls[1].arguments.contains("files.example.invalid"));
        assert!(!calls[1].arguments.contains("labuser"));
        assert!(!calls[1].arguments.contains("hunter2"));
    }
