...
3. `gh workflow run "Publish Release" --ref main` を実行し、`gh run list --workflow="Publish Release" --limit 1 --json databaseId,url,status,displayTitle` で URL を取得する。必要なら `gh run watch <databaseId>`、失敗時は `gh run view <databaseId> --log-failed`。Write 権限だけでは失敗する。CI は配布 DMG を notarize / staple してから最終 verify し、成功するまで Release は draft のまま。
4. 対象バージョン、マージした PR、ワークフロー URL を報告する。成功後は `v<version>` タグ、公開 GitHub Release、Apple Silicon の署名済み .dmg が CI から続くことも伝える。
