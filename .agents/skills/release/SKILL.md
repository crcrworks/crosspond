...
3. `gh workflow run "Publish Release" --ref main` を実行し、`gh run list --workflow="Publish Release" --limit 1 --json databaseId,url,status,displayTitle` で URL を取得する。必要なら `gh run watch <databaseId>`、失敗時は `gh run view <databaseId> --log-failed`。Write 権限だけでは失敗する（repository admin が `release` environment で起動する）。
4. 対象バージョン、マージした PR、ワークフロー URL を報告する。成功後は `v<version>` タグ、GitHub Release、Apple Silicon の .dmg アップロードが CI から続くことも伝える。
