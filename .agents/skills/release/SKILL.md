...
3. `gh workflow run "Publish Release"` を実行し、`gh run list --workflow="Publish Release" --limit 1 --json databaseId,url,status,displayTitle` で URL を取得する。必要なら `gh run watch <databaseId>`、失敗時は `gh run view <databaseId> --log-failed`。
